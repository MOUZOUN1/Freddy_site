        ...
        <form method="POST" action="{{ route('admin.regions.update', $region) }}">
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="4"
                          class="w-full px-4 py-2 border rounded-lg @error('description') border-red-500 @enderror">{{ old('description', $region->description) }}</textarea>
                @error('description')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="mb-6">
                <label class="block text-gray-700 font-medium mb-2">Localisation</label>
                <input type="text" name="localisation" value="{{ old('localisation', $region->localisation) }}"
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" name="superficie" value="{{ old('superficie', $region->superficie) }}"
                ...
                <div>
                    ...
                    <input type="number" name="population" value="{{ old('population', $region->population) }}"
